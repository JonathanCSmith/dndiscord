...
class ServicesManager(Module):
    ...
    @commands.command("services:rest")
    async def _rest(self, ctx: commands.Context):
        # Do we have permission to run this command. A game must be running.
        if not await self.game_master.check_active_game_permissions_for_user(ctx, "services:rest", permissions_level=constants.open):
        ...
        await ctx.send("For this rest you party will need to consume: " + player_count + " rations plus any companions & animals.")
    ...
    @commands.command("services:inn:rest")
    async def _rest(self, ctx: commands.Context, *, cost_in_cp: int):
        # Do we have permission to run this command. A game must be running.
        if not await self.game_master.check_active_game_permissions_for_user(ctx, "services:inn:rest", permissions_level=constants.open):
            return await ctx.send("`You do not have permission to run that command or a game is not running.`")
    ...
    @commands.command("services:inn:eat")
    async def _rest(self, ctx: commands.Context, *, cost_in_cp: int):
        # Do we have permission to run this command. A game must be running.
        if not await self.game_master.check_active_game_permissions_for_user(ctx, "services:inn:eat", permissions_level=constants.open):
            return await ctx.send("`You do not have permission to run that command or a game is not running.`")
        ...
        # Check for inventory integrations
        await ctx.send("For this food you party will need : " + cost + " copper pieces per head not including companions and stabling.")
